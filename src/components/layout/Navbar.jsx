...
import { Menu, X, ChevronDown, Home, Users, Briefcase, BarChart2, Calendar, Image, ShieldCheck, FileText, MessageCircle, Info, FolderHeart as HandHeart, Award, Newspaper, BookHeart, Landmark, Leaf, FileHeart as FileShield, Rss, Sparkles } from 'lucide-react';
...
const Navbar = ({ logoUrl, handleDonateToast }) => {
  ...
  return (
    <motion.nav 
      ...
      ${isScrolledEnough || isMenuOpen ? 'bg-white/95 backdrop-blur-lg shadow-xl border-b border-slate-200/60' : 'bg-transparent'}`}
    >
      <motion.div 
          ...
          <Link to="/" className="flex items-center space-x-2.5 shrink-0 group">
            ...
            />
             {(!isMenuOpen || !isScrolledEnough) && (
                <motion.span 
                  className="text-2xl md:text-3xl font-bold gradient-text hidden sm:inline tracking-tight group-hover:opacity-80 transition-opacity"
                  initial={{ opacity: 1 }}
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div 
            ...
            className="lg:hidden bg-white/98 backdrop-blur-lg shadow-2xl border-t border-slate-200/50 absolute w-full"
            style={{ maxHeight: `calc(100vh - ${initialNavHeight})`, overflowY: 'auto' }}
          >
            <div className="px-3 pt-3 pb-5 space-y-1.5 sm:px-4">
              <div className="flex items-center justify-between px-3 pt-2 pb-4 border-b border-slate-200/70 mb-2 relative">
