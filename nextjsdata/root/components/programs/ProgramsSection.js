...
const ProgramCard = ({ icon: Icon, title, description, linkTo, delay, iconBgColor, borderColor, imagePlaceholder }) => (
  ...
  >
    ...
    <div className="p-6 flex flex-col flex-grow">
      <h3 className="text-xl font-bold text-dream-deep-purple mb-3">{title}</h3>
      ...
      <Link href={linkTo} className="mt-auto self-start">
        <button className={`${borderColor.replace('border-', 'text-').replace('border-dream-purple', 'text-dream-purple')} px-0 group/btn font-medium hover:text-dream-gold transition-colors flex items-center`}>
    ...
    <section id="programs" className="py-20 md:py-28 bg-slate-50 overflow-hidden">
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mt-14 md:mt-18">
            <Link href="/programs">
                <button className="gradient-bg-warm text-white hover:opacity-90 rounded-full px-10 py-3.5 text-base font-semibold shadow-lg hover:shadow-xl transition-transform transform hover:scale-105 group flex items-center mx-auto">
