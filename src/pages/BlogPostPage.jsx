  ...
  if (!post) {
    return (
      <div className="text-center py-20">
        <h1 className="text-2xl font-bold text-secondary-foreground">Post not found</h1>
  ...
  return (
    <div className="bg-slate-50">
      <PageHeader
        ...
        icon={Tag} 
        gradientFrom="from-dream-logo-blue-light"
      ...
      <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        ...
        >
          <Link to="/blog" className="inline-flex items-center text-sm text-primary hover:text-secondary mb-6 group">
            <ArrowLeft size={18} className="mr-1.5 group-hover:-translate-x-1 transition-transform" />
            Back to All Articles
          </Link>

          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-secondary-foreground mb-4 leading-tight">{post.title}</h1>
          
          <div className="flex flex-wrap items-center text-sm text-slate-500 mb-8 gap-x-4 gap-y-2">
            <span className="flex items-center"><CalendarDays size={16} className="mr-1.5 text-primary"/> {post.date}</span>
            <span className="flex items-center"><User size={16} className="mr-1.5 text-primary"/> By {post.author}</span>
            <span className="flex items-center"><Tag size={16} className="mr-1.5 text-primary"/> {post.category}</span>
          </div>

          <img  class="w-full h-auto max-h-[500px] object-cover rounded-2xl shadow-xl mb-10" alt={post.title} src={post.imageSrc} />

          <div 
            className="prose prose-lg max-w-none text-slate-700 leading-relaxed prose-headings:text-secondary-foreground prose-a:text-primary hover:prose-a:text-secondary prose-strong:text-secondary-foreground"
            dangerouslySetInnerHTML={{ __html: post.content }} 
          />

          <div className="mt-12 pt-8 border-t border-slate-200">
            <h3 className="text-lg font-semibold text-secondary-foreground mb-3">Share this article:</h3>
