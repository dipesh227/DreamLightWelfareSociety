...
const StatCard = ({ value, label, icon: Icon, color }) => (
  ...
  >
    ...
    <p className="text-3xl font-bold text-secondary-foreground">{value}</p>
...
const TimelineEvent = ({ year, title, description, alignLeft, index }) => (
  ...
  >
    ...
    <div className="hidden md:flex w-1/12 justify-center">
      <div className="h-full w-1 bg-dream-logo-blue-light rounded-t-full rounded-b-full"></div>
    </div>
    <div className="w-full md:w-5/12">
      <div className={`bg-white p-6 rounded-xl shadow-xl relative ${alignLeft ? 'md:mr-auto' : 'md:ml-auto'}`}>
        <div className={`absolute ${alignLeft ? 'md:right-full md:-mr-5' : 'md:left-full md:-ml-5'} top-1/2 -translate-y-1/2 hidden md:block`}>
          <div className="bg-primary text-white w-10 h-10 rounded-full flex items-center justify-center font-bold shadow-md">
            {year}
          </div>
        </div>
        <div className="md:hidden bg-primary text-white w-10 h-10 rounded-full flex items-center justify-center font-bold shadow-md mb-3">{year}</div>
        <h3 className="text-lg font-semibold text-secondary-foreground mb-1">{title}</h3>
...
const AboutPage = () => {
  ...
  return (
    <div className="bg-slate-50">
      <PageHeader 
        title="Our Story of Change in Uttarakhand"
        subtitle="Dreamlight Welfare Society, founded on June 4th, 2025 (Reg. No. TRSOCI067080525216401), is dedicated to empowering communities in Uttarakhand."
        icon={Info}
        ...
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-12 items-center mb-16 md:mb-24">
            ...
            <motion.div
              ...
              className="space-y-6"
            >
              <h2 className="text-3xl md:text-4xl font-bold text-secondary-foreground mb-4">Our Purpose & Commitment</h2>
              ...
              <ul className="space-y-2 text-slate-700 text-sm">
                <li className="flex items-center"><Users className="mr-2 h-4 w-4 text-primary" /> Women Empowerment (महिला विकास) & Child Development (बाल विकास)</li>
                <li className="flex items-center"><Lightbulb className="mr-2 h-4 w-4 text-primary" /> Skill Training: Handicrafts, Computers, Self-Employment (कौशल प्रशिक्षण)</li>
                <li className="flex items-center"><HandHeart className="mr-2 h-4 w-4 text-primary" /> Health Awareness: AIDS Prevention, Drug De-addiction (स्वास्थ्य जागरूकता)</li>
                <li className="flex items-center"><Leaf className="mr-2 h-4 w-4 text-primary" /> Environmental Protection & Awareness (पर्यावरण संरक्षण)</li>
              </ul>
              <Link to="/our-values">
          ...
          <div className="text-center mb-16 md:mb-24">
            <h2 className="text-3xl md:text-4xl font-bold text-secondary-foreground mb-12">Our Journey: Beginnings & Aspirations</h2>
            <div className="relative wrap overflow-hidden p-2 md:p-10 h-full">
              <div className="absolute h-full border border-dream-logo-blue-light/50 border-dashed" style={{left: '50%', display: 'none' /* Hide for now, enable for desktop timeline */}}></div>
          ...
          <div className="text-center mb-16 md:mb-24">
            <h2 className="text-3xl md:text-4xl font-bold text-secondary-foreground mb-12">Our Commitments</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
              <StatCard value="New" label="Organization (Est. 2025)" icon={CalendarClock} color="text-secondary" />
              <StatCard value="Local" label="Focus on Uttarakhand" icon={Users} color="text-primary" />
              <StatCard value="Core" label="Objectives Defined" icon={ShieldCheck} color="text-dream-logo-blue-light" />
              <StatCard value="Future" label="Growth & Impact" icon={Target} color="text-green-500" />
            </div>
          </div>

          <div className="text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-secondary-foreground mb-6">Meet Our Founding Team</h2>
