...
import { Users, BookOpen, Stethoscope, Globe, Star } from 'lucide-react'; // Added Star here

const ImpactStatCard = ({ icon: Icon, value, label, iconColor, bgColor }) => (
  <motion.div 
    whileHover={{ scale: 1.05 }}
    className="scroll-reveal text-center bg-white p-6 md:p-8 rounded-2xl shadow-lg card-hover"
  >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-12 items-center">
          <div className="scroll-reveal">
            <img  alt="Happy children benefiting from NGO programs" className="rounded-2xl shadow-2xl object-cover w-full h-full max-h-[500px]" src="https://images.unsplash.com/photo-1502086223501-7ea6ecd79368" />
          </div>

          <div className="scroll-reveal space-y-6">
            <h3 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">Success Stories</h3>
            {testimonials.map((testimonial, index) => (
              <motion.div 
                key={index} 
                className="bg-white p-6 rounded-xl shadow-lg glass-effect border border-dream-purple/20"
              ...
              >
                ...
                <p className="text-gray-700 mb-3 text-shadow-sm">"{testimonial.quote}"</p>
                <p className="font-semibold text-dream-purple-dark">- {testimonial.author}</p>
              </motion.div>
            ))}
          </div>
        </div>
