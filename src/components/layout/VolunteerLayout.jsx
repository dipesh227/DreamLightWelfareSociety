...
const VolunteerSidebarLink = ({ to, icon: Icon, label, isCollapsed, isActive, isDarkMode }) => (
  <Link
    ...
    className={`flex items-center px-3 py-3.5 rounded-lg transition-colors duration-200 ease-in-out group
      ...
      ${isActive
        ? 'bg-secondary text-secondary-foreground shadow-md hover:bg-dream-logo-yellow-dark'
        : isDarkMode
          ? 'text-gray-300 hover:bg-gray-700 hover:text-secondary'
          : 'text-slate-600 hover:bg-dream-logo-yellow/10 hover:text-secondary'
      }`}
    title={isCollapsed ? label : undefined}
  >
    <Icon className={`h-5 w-5 ${isCollapsed ? '' : 'mr-3'} ${isActive ? 'text-secondary-foreground' : isDarkMode ? 'text-gray-400 group-hover:text-secondary' : 'text-slate-500 group-hover:text-secondary'}`} />
        ...
        <div className={`flex items-center p-4 ${isDarkMode ? 'border-gray-700' : 'border-slate-200'} border-b h-[4.5rem] ${isSidebarCollapsed ? 'justify-center' : 'justify-between'}`}>
          {!isSidebarCollapsed && (
            <Link to="/volunteer-dashboard/overview" className="flex items-center space-x-2 overflow-hidden">
              ...
              <span className={`font-bold text-lg whitespace-nowrap ${isDarkMode ? 'text-secondary' : 'text-secondary'}`}>Volunteer Hub</span>
            </Link>
          )}
           <Button 
            variant="ghost" 
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-slate-200'} border-b shadow-sm h-[4.5rem] flex items-center justify-between px-6 md:px-8 print:hidden`}>
          ...
          <div className="flex items-center space-x-3">
            ...
            <Link to="/">
              <Button variant="outline" size="sm" className={`${isDarkMode ? 'text-secondary border-secondary hover:bg-secondary hover:text-gray-900' : 'text-secondary border-secondary hover:bg-secondary hover:text-white'}`}>
