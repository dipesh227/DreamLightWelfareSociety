...
interface ContactInfoItemBigProps {
...
const ContactInfoItemBig: React.FC<ContactInfoItemBigProps> = ({ icon: Icon, title, lines, iconBgColor, iconColor }) => (
  <motion.div
    className="bg-card p-6 rounded-xl shadow-lg text-center flex flex-col items-center card-hover"
    whileHover={{ y: -5 }}
  ...
  >
    ...
    {lines.map((line, index) => <p key={index} className="text-muted-foreground text-sm">{line}</p>)}
  </motion.div>
);
    ...
    <section className="py-16 md:py-24">
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          <motion.div
            className="bg-card p-8 md:p-10 rounded-2xl shadow-xl"
            ...
            <form className="space-y-6" onSubmit={handleFormSubmit}>
              <div className="grid sm:grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="text" id="name" name="name" required className="w-full p-3 border border-border rounded-lg bg-background text-foreground focus:ring-2 focus:ring-dream-purple focus:border-transparent transition-all duration-200" placeholder="e.g., John Doe" />
                </div>
                <div>
                  <label htmlFor="email" className="block text-sm font-semibold text-foreground mb-1.5">Email Address</label>
                  <input type="email" id="email" name="email" required className="w-full p-3 border border-border rounded-lg bg-background text-foreground focus:ring-2 focus:ring-dream-purple focus:border-transparent transition-all duration-200" placeholder="you@example.com" />
                </div>
              </div>
              <div>
                <label htmlFor="subject" className="block text-sm font-semibold text-foreground mb-1.5">Subject</label>
                <input type="text" id="subject" name="subject" required className="w-full p-3 border border-border rounded-lg bg-background text-foreground focus:ring-2 focus:ring-dream-purple focus:border-transparent transition-all duration-200" placeholder="Reason for your message" />
              </div>
              <div>
                <label htmlFor="message" className="block text-sm font-semibold text-foreground mb-1.5">Your Message</label>
                <textarea id="message" name="message" rows={5} required className="w-full p-3 border border-border rounded-lg bg-background text-foreground focus:ring-2 focus:ring-dream-purple focus:border-transparent transition-all duration-200 resize-none" placeholder="Write your message here..."></textarea>
              </div>
              <Button type="submit" size="lg" className="w-full gradient-bg text-white text-base py-3 rounded-lg hover:opacity-90 shadow-lg transition-all duration-200">
