...
const OpportunityCard = ({ opportunity, onApply }) => (
  <motion.div
    className="bg-white p-6 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 border-l-4 border-dws-gold flex flex-col"
    initial={{ opacity: 0, y: 20 }}
    whileInView={{ opacity: 1, y: 0 }}
    viewport={{ once: true, amount: 0.3 }}
    transition={{ duration: 0.5 }}
  >
    <h3 className="text-lg font-bold text-dws-blue-dark mb-2">{opportunity.title}</h3>
    <div className="text-xs text-slate-500 mb-3 space-y-0.5">
      <p><span className="font-semibold text-dws-blue-dark">Area:</span> {opportunity.area}</p>
      <p><span className="font-semibold text-dws-blue-dark">Location:</span> {opportunity.location}</p>
      <p><span className="font-semibold text-dws-blue-dark">Commitment:</span> {opportunity.commitment}</p>
    </div>
    ...
    <div className="mb-3">
      ...
      <div className="flex flex-wrap gap-1.5">
        {opportunity.skills.map(skill => (
          <span key={skill} className="bg-dws-blue-light/20 text-dws-blue-dark text-[10px] px-2 py-0.5 rounded-full">{skill}</span>
        ))}
      </div>
    </div>
...
const VolunteerSpotlightCard = ({ name, role, quote, imageUrl, index }) => (
  <motion.div
    className="bg-dws-blue-light/10 p-6 rounded-xl shadow-lg text-center"
    initial={{ opacity: 0, y: 20 }}
    whileInView={{ opacity: 1, y: 0 }}
    viewport={{ once: true, amount: 0.3 }}
  ...
  >
    ...
    <h4 className="text-lg font-semibold text-dws-blue-dark">{name}</h4>
    <p className="text-xs text-dws-blue-dark mb-2">{role}</p>
    <Star className="w-5 h-5 text-dws-gold mx-auto mb-2 fill-dws-gold" />
...
const VolunteerPortalPage = () => {
  ...
  const handleApply = (title) => {
    toast({
      ...
      className: "bg-dws-blue-dark text-white border-dws-gold shadow-xl",
    });
  };
  ...
  return (
    <div className="bg-slate-100">
      <PageHeader
        ...
        subtitle="Join our passionate community of volunteers and make a real difference. Find opportunities, manage your engagement, and be part of the Dreamlight family."
        icon={HandHeart}
      ...
      <section id="opportunities" className="py-16 md:py-24">
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-dws-blue-dark mb-3">Current Volunteer Opportunities</h2>
          ...
          <div className="mb-10 max-w-lg mx-auto">
            <div className="relative">
              <input 
                ...
                className="w-full p-3.5 pl-10 border border-slate-300 rounded-lg focus:ring-2 focus:ring-dws-blue-dark focus:border-transparent transition-shadow"
              />
      ...
      <section id="volunteer-spotlight" className="py-16 md:py-24 bg-white">
        <div className="max-w-screen-lg mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-dws-blue-dark mb-3">Volunteer Spotlight</h2>
      ...
      <section id="id-card" className="py-16 md:py-24 bg-dws-gold/10">
        <div className="max-w-screen-md mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <IdCard className="h-16 w-16 mx-auto text-dws-blue-dark mb-6" />
          <h2 className="text-3xl md:text-4xl font-bold text-dws-blue-dark mb-4">Volunteer ID Cards</h2>
          ...
          <div className="bg-white p-8 rounded-xl shadow-lg text-left space-y-4 max-w-lg mx-auto">
            <h4 className="text-xl font-semibold text-dws-blue-dark mb-3">How to Get Your ID Card (Simulated):</h4>
            <p className="flex items-start text-sm text-slate-600"><CheckCircle size={18} className="mr-2 mt-0.5 text-dws-green flex-shrink-0"/>Successfully complete the volunteer registration and onboarding process.</p>
            <p className="flex items-start text-sm text-slate-600"><CheckCircle size={18} className="mr-2 mt-0.5 text-dws-green flex-shrink-0"/>Attend a mandatory orientation session (online or in-person).</p>
            <p className="flex items-start text-sm text-slate-600"><CheckCircle size={18} className="mr-2 mt-0.5 text-dws-green flex-shrink-0"/>Submit a passport-sized photograph through your volunteer dashboard (coming soon).</p>
            <p className="flex items-start text-sm text-slate-600"><CheckCircle size={18} className="mr-2 mt-0.5 text-dws-green flex-shrink-0"/>Your ID card will be processed and mailed to you or available for pickup at our office/events.</p>
      ...
      <section className="py-20 bg-dws-blue-dark text-white">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <motion.div
            ...
            whileTap={{ scale: 0.95 }}
          >
            <Button asChild size="lg" className="bg-dws-gold text-dws-blue-dark hover:bg-dws-gold-light font-semibold px-10 py-3.5 rounded-full text-base shadow-lg hover:shadow-xl transition-all duration-300">
