...
const NavLinkItem = ({ to, label, icon: Icon, mobile = false, onClick, className = '' }) => {
  ...
  return (
    <Link
      ...
      className={`flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 ease-in-out group ${className}
        ${location.pathname === to 
          ? 'text-dream-purple-dark bg-dream-purple-lighter font-semibold shadow-sm' 
    ...
    >
      {Icon && <Icon className={`mr-2.5 h-5 w-5 ${location.pathname === to ? 'text-dream-purple' : 'text-slate-500 group-hover:text-dream-purple'}`} />}
      {label}
    </Link>
  );
};
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute left-0 mt-2 w-64 bg-white rounded-xl shadow-2xl py-2 z-20 border border-slate-200/80"
          >
            {subLinks.map(link => 
              link.subLinks ? 
              <div key={link.label} className="relative group/submenu">
                 <button className="flex items-center justify-between w-full px-4 py-2.5 text-sm text-slate-700 hover:bg-dream-purple-lighter/70 hover:text-dream-purple rounded-md">
                  ...
                  <motion.div 
                    ...
                    exit={{ opacity: 0, x: -10 }}
                    className="absolute left-full top-[-8px] ml-1 w-60 bg-white rounded-xl shadow-2xl py-2 z-30 border border-slate-200/80 hidden group-hover/submenu:block"
                  >
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div 
            ...
            className="lg:hidden bg-white/98 backdrop-blur-xl shadow-2xl border-t border-slate-200/60 absolute w-full"
            style={{ maxHeight: `calc(100vh - ${scrolledNavHeight})`, overflowY: 'auto' }}
          >
            <div className="px-3 pt-3 pb-5 space-y-1.5 sm:px-4">
              <div className="flex items-center justify-between px-3 pt-2 pb-4 border-b border-slate-200/80 mb-2 relative">
                  <Link to="/" className="flex items-center space-x-2.5 -ml-1" onClick={closeMobileMenu}>
                    <motion.img 
                      src={logoUrl} 
                   ...
                   <button 
                    ...
                    className="p-2 rounded-lg text-slate-500 hover:bg-slate-100 absolute top-3 right-2"
                  >
                    <X className="h-6 w-6" />
                  </button>
              </div>
